...
public class PlayerMovement : MonoBehaviour
{
    ...
    private bool TryLightAttack()
    {
        ...
        ScheduleMeleeHit(lightAttackLockTime * 0.45f);
    ...
    private bool TryHeavyAttack()
    {
        ...
        ScheduleMeleeHit(heavyAttackLockTime * 0.45f);
    ...
    private void ScheduleMeleeHit(float delay)
    {
        PlayerStats ps = GetComponent<PlayerStats>() ?? GetComponentInParent<PlayerStats>();
        pendingAttackDamage = ps != null ? ps.damage : 10f;
        meleeHitTime = Time.time + delay;
    ...
    private void CheckPendingMeleeHit()
    {
        ...
        Vector3 origin = movementRoot.position + movementRoot.forward * meleeHitForwardOffset
                         + Vector3.up;
        ...
        foreach (Collider hit in hits)
        {
            ...
            float reduction = ai != null ? pendingAttackDamage * (1f - ai.BlockDamageMultiplier) : 0f;
            enemy.TakeDamage(pendingAttackDamage, reduction);
        }
    }
